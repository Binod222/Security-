...
import { toast } from "react-toastify";
import Loader from "../../component/Loader";
import { useGetProfileQuery, useUpdateProfileMutation } from "../../redux/api/users";
import { setCredentials } from "../../redux/features/auth/authSlice";
...
const Profile = () => {
  ...
  // Fetch profile data
  const { data: userInfo, isLoading, error } = useGetProfileQuery();
  ...
  // When userInfo loads or changes, update form fields
  useEffect(() => {
    if (userInfo) {
      setUsername(userInfo.username);
      setEmail(userInfo.email);
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const res = await updateProfile({
        _id: userInfo._id, // or userInfo.id depending on your API
        username,
        email,
        password,
      }).unwrap();
